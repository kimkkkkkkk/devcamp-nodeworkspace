...
[express 모듈의 특징]
1) 정적자원에 대한 처리가 이미 지원된다.. 즉 개발자가 각 자원마다 1:1 대응하는 코드를 작성할 필요가 없다
    html, image, css, soundfile, mp4, js(클라이언트 즉),txt 서버에서 실행되지 않는 모든 리소스!~
2) Get/Post 등의 http요청에 대한 파라미터 처리가 쉽다
3) 미들웨어라 불리는 use() 메서드를 통해 기능을 확장한다!!
*/

var http=require("http");// express는 http에 얹혀쓰인다
...
//서버의 정적자원에 접근을 위해 static() 미들웨어를 사용해본다!! static(정적인)<---> dynamic(동적)
//__direname 전역변수는? 현제 실행중인 js 파일의 디렉토리 위치를 반환
// 즉 현재 실행중인 server.js 의 디렉토리를 반환!!
//console.log("이미지 정적 자원이 들어있는 풀 경로는 ", __dirname+"/image");
app.use(static(__dirname+"/static")); //static을 정적차원이 있는 루트로 지정
...
/*--------------------------------------------------------------------------------------------------------------------
클라이언트의 요청 처리!!! app.post(); , app.get(); , 요청 url에 대한 조건문x 정적자원에 대한 처리필요x
DML 업무를 CRUD 얘기하기도 함 -
Create(=insert), Read(=select), Update, Delete
----------------------------------------------------------------------------------------------------------------------*/
app.get("/notice/list", function(request, response){
   var con = mysql.createConnection(conStr);//접속 시도 후 Connection 객체 반환
    //select 문 수행하자
    con.query("select * from notice order by notice_id desc", function(error, result, fields){ // fields 안에는 각 레코드마다 컬럼의 사이즈 메타 정보가 들어있다.
        if(error){
            console.log(error); // 에러 분석을 위해 콘솔화면에 로그를 남김
        }else{
            //console.log("result는 ", result); 찍어보기
            ...
            fs.readFile("./notice/list.ejs", "utf8", function(err, data){
                ...
                response.end(ejs.render(data,{ // 데이터 뒤에 json 
                   noticeList:result, // result는 json으로 된 구조
                    lib:mymodule
                }));
            });
        }    
        con.end(); // *중요: mysql과 접속 끊기
    });
});


//지정한 url의 post 방식으로 클라이언트의 요청을 받는다.
app.post("/notice/regist", function(request, response){
    //1)클라이언트가 전송한 파라미터들을 받자!!
    console.log(request.body); // 얘 자체가 json임
    var title=request.body.title;
    var writer=request.body.writer;
    var content=request.body.content;

    //2) mysql 접속 후 Connection 객체 변환
...
app.get("/notice/detail", function(request, response){
    //console.log(request.query); // json으로 나타남
                    ...
                    fs.readFile("./notice/detail.ejs", "utf8", function(error, data){
                        if(error){
                            ...
                            response.end(ejs.render(data, {
                                //result는 한건이라고 할 지라도 배열이므로, 배열에서 꺼내서 보내주자
                                record:result[0]
                            }));
                        }
                    });
                }
                con.end(); // mysql과 접속 끊기 121번 괄호 ,헷갈리면 선따라 보기
            });
        }
    });
});
