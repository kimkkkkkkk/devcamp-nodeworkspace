...
var server = http.createServer(function(request, response){
    ...
    //보내보자!!  request가 포인트* 클라이언트의 요청 정보가 다 들어있음
    ...
    var url=request.url; // 클라이언트가 어떤 주소로 들어왔는지 알수있음
    console.log("클라이언트가 요청시 입력한 주소는", url);


    //localhost:8888/member/form : 회원가입 디자인폼을 요청으로 간주
    ...
    //회원가입을 희망하는 사람이라면....
        fs.readFile("./regist_form.html", "utf8", function(error, data){ 
           // 파일의 내용을 모두 읽어들인 순간 이 익명함수가 동작함
            response.end(data); // 응답하기 :쌍따옴표에 넣는 데이터만이 응답정보가 나옴 
        });
