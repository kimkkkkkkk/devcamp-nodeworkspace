...
//(쪼개져서) 직렬화된 전송한 데이터에 대한 해석을 담당(문자열로 해석가능)
var qs=require("querystring"); // 왜쓰이는지 알아야함
var url=require("url"); //url분석과 관련된 내부모듈
var mysql=require("mysql"); //mysql 모듈 가져오기 (외부모듈이므로 별도 설치 필요)
var ejs = require("ejs");//ejs 모듈을 가져오기 (외부모듈) 매우 중요
...
var server = http.createServer(function(request, response){
    ...
    */
    ...
    switch(requestUrl){
        ...
        case "/member/list": getList2(request, response);break; // getList 2 로 바뀜
...
// 1:1로 대응되는 요청을 처리한다면..... 개발에 효율성도 없고,,, 안하고 만다
//결론: 정적 자원 처리뿐만 아니라 개발의 편의성을 위해 좀더 개선된 서버 모듈로 전환!!
function getImage(request, response){
    fs.readFile("./image/f.jpg", function(err, data){
...
function regist(request, response){
    ...
    // param에는 body안에 들어있는 데이터가 서버의 메모리 버퍼로 들어오고, 그 데이터를 paramd 담고있다.
    ...
    con.query(sql, function(err, fields){
            if(err){ // 쿼리수행중 심각한 에러발생, 에러가 발생한게 참이라면
            ...
            //db작업 성공 여부와 상관없이 연결된 접속은 끊어야한다!!
            con.end();// 접속 끊기!
        }); // 쿼리가 실행된 시점 (실패,성공 언제 할지 모름 그래서 익명함수 줌)
    }); 
}

//아래의 getList()보다 더 대선된 방법으로 요청을 처리하기 위해, 함수를 별도로 정의한 것임!!
function getList2(request, response){
    //클라이언트에게 결과를 보여주기 전에, 이미 DB연동을 하여 레코드를 가져와야한다!!
    var con = mysql.createConnection(conStr); // 접속
    var sql = "select * from member";
    con.query(sql, function(err, record, fields){
        //record 변수엔 json들이 일차원 배열에 탑재되어 있다.
        //console.log("record is..", record);// ********목록확인 잠깐 막기
        //파일을 모두 읽으면 익명함수가 호출되고, 이 익명함수안에 매개변수에 읽혀진 모든 데이터가
        //매개변수로 전달된다!!

         //con query메서드가 수행을 완료되는 시점은 익명혐수 안이다
        //익명함수는 con query메서드가 완료되었을때 function 영역을 수행한다.
        fs.readFile("./list.ejs","utf8", function(err, data){
            if(err){
                console.log("list.ejs을 읽는데 실패!!");
            }else{
                response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
               //클라이언트에게 list.ejs를 그냥 그대로 보내지말고, 서버에서 실행을 시킨 후 , 그 결과를
               //클라이언트에게 전송한다!!
               //즉, ejs를 서버에서 렌더링 시켜야 한다!!
               
                var result = ejs.render(data, {
                   //member:누구?? db테이블의 레코드를 표현한 배열
                    members:record
               }); // 퍼센트 안에 들어있는 코드가 실행되버린다!!
                response.end(result);
            }
        });
    // getList2 이 안은 순서가 없음 (콜벡은 비동기:동기화되어 있지 않다)
    });//형식, SQL문, 결과레코드, 필드정보
...
//이 방법은 디자인 마저도, 프로그램 코드에서 감당하고 있기 때문에
//유지보수성이 너무 낮다.. 따라서 프로그램 코드와 디자인은 분리되어야 좋다!!
//따라서 아래의 방법은 두번다시는 사용하지 않겠다..(개발 규모가 좀 커질때)
function getList(request, response){
    //회원목록 가져오기!!
    //연결된 db 커넥션이 없으므로, mysql에 재 접속하자
    var con=mysql.createConnection(conStr); // 여기서 접속이 됨
    var sql="select * from member";

    //2번째 인수 : select 문 수행결과 배열
    //3번째 인수 : 컬럼에 대한 메타 정보(메타 데이터란? 정보자체에 대한 정보이다.)
    //                  여기서는 컬럼의 자료형, 크기 등에 대한 정보..
    con.query(sql, function(err, result, fields){
        // console.log("쿼리문 수행후 mysql로부터 받아온 데이터는", result);
        //result 분석하기!!
        //console.log("컬럼정보 ", fields); //실제 레코드에 대한 정보
        // console.log("결과 레코드 수는 ", result.length); // 컬럼, 필드, 글씨는 몇자이며 등등에 대한 자료
...
function getDetail(request, response){
    ...
    //겟방식으로 날라옴, 겟방식은 노출됨 , 직관적이다
    
    
    //개발자가 직접 url을 문자열 분석을 시도하기 보다는 url을 전문적으로 해석 및 분석할 수 있는
    //모듈에게 맡기면 된다!! 그 역할을 수행하는 모듈이 바로 url 모듈 이다!!
    // querystring : post 방식의 파라미터 추출
    // url : get 방식의 파라미터 추출
    var param=url.parse(request.url,true).query; // true을 매개변수로 넘기면,
                                                                      // json 형채로 파라미터를 반환해줌
    
    console.log("상세보기에 필요한 추출 파라미터는 ",param); // 반환형은 제이슨
    ...
    //쿼리문 수행하고 readFile 되어야 함
     con.query(sql, function(err, result, fields){
        console.log(result);
        //쿼리문이 수행 완료된 시점이므로, 이때 사용자에게 상세페이지를 보여준다!!
        fs.readFile("./detail.ejs", "utf8", function(err, data){ // html 파일이였다가 ejs로 바뀜
            if(err){
                console.log(err);
            }else{
                //클라이언트에게 html을 읽어들인 내용을 보내주자!
                response.writeHead(200, {"Content-type":"text/html;charset=utf-8"});
                //render의 대상은 ejs 파일내의 %%퍼센트 영역만 골라먹는다!
                //따라서 모든 render작업이 끝나면 html로 재구성하여 응답정보에 실어서
                //클라이언트에게 응답을 실행한다!!
                response.end(ejs.render(data,{
                    //한건이라 할지라도, select 문의 결과는 배열이다!! 따라서 0번째 추출해서 보내자
                    record:result[0] // 한사람의 정보를  ejs파일로 가져가자, record는 변수
                }));
            }
            con.end(); //mysql과의 접속 끊기
        }); // ↑callback 함수 익명함수를 나타낸다
    });
}
    /*  ↑쿼리문과 readfile 중 뭐가 먼저실행될까? 정답은 모른다, 익명함수로 인해  쿼리문에서 그냥 지나가버림
    쿼리문과 readfile 둘다 callblak 임 
    쿼리문 이 완료된 후 내용을 보여줘야함 내용을 보여줘야 하는 코드는 어디로 가야하나면 쿼리문의 익명함수 callback에 넣어줘야함
    */
...
function edit(request, response){
    ...
    //글 수정은 클라이언트로부터 post 방식으로 전송되기 때문에, 그 데이터가 body에 들어있다.
    //body에 들어있는 파라미터를 추출하기 위한 모듈이 querystring이다!!

    //post 방식의 데이터는 버퍼에 담겨오기 때문에, 이 따로 따로 직렬화되어 분산된 데이터를
    //일단 문자열로 모아서 처리해야 한다
    var content="";
    ...
    request.on("end", function(){
        ...
        sql+="where member_id="+obj.member_id; // sql+= 연결해준것  너무길어서
       
        var con=mysql.createConnection(conStr); //쿼리문 실행하려면 '접속'을 먼저 한다. 그리고 반환형 var con =
...
function del(request, response){
    ...
    //커넥션 객체는 접속 정보를 가진 객체이며, 이 객체를 통해 쿼리를 수행할 수 있따.. 또한 mysql 접속을 해제
    //도 가능하다
    //select문의 경우엔 결과를 가져와야 하기 때문에 가운데 인수, result인수가 필요하지만
    //delete, update, insert DML은 가져올 레코드가 없기때문에 인수가 2개이면 충분함
    con.query(sql, function(err, fields){   //가운데 result값을 뺸 이유 : 결과를 받을 일이 없어서
        if(err){
            console.log(err);
        }else{
            response.writeHead(200, {"Content-type":"text/html;charset=utf-8"});
